...
class BreastCancerDataset(torch.utils.data.Dataset):
    ...
    def __getitem__(self, index):  # Gets each image from path
        ...
        process_function_x = T.Compose([
            ...
            ), T.RandomAutocontrast(), T.RandomEqualize(), T.RandomAdjustSharpness(sharpness_factor=2)
        ])
...
class TestBreastCancerDataset(torch.utils.data.Dataset):
    def __init__(self, paths):
        self.paths = paths
    ...
    def __getitem__(self, index):  # Gets each image from path
        sample = self.paths[index]
        load_dotenv('.env')
        x_env_path = os.getenv("Images_Test")
        y_env_path = os.getenv("Masks_Test")
